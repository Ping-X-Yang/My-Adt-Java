...
//	   IProjectInfo projectInfo;
	...
	private Button requestObjectVisibledCheckBox;
...
//	      this.setMessage(descriptionMessage, 1);
	...
	private void addOtherGroup() {
		...
		GridDataFactory.fillDefaults().indent(0, 5).grab(true, false).span(4, 1).applyTo(group);
		this.addRequestObjectVisibledCheckBox(group);	//请求对象可见
	}

	private void addRequestObjectVisibledCheckBox(Composite composite) {
		this.setRequestObjectVisible(this.preferenceStoreModel.isRequestObjectVisible());
		this.requestObjectVisibledCheckBox = new Button(composite, 32);
		this.requestObjectVisibledCheckBox.setText("请求对象可见");
		this.requestObjectVisibledCheckBox.setSelection(this.preferenceStoreModel.isRequestObjectVisible());
		this.requestObjectVisibledCheckBox.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				Button btn = (Button) e.getSource();
				setRequestObjectVisible(btn.getSelection());
			}
		});
	}
	...
	private void addRequestStatusGroup() {
		...
		requestStatusGroup.setTabList(new Control[] { this.modifiable, this.released, this.filterReleasedDateCombo,
				blankLabel1, blankLabel2, fromDateComposite, toDateComposite });
	...
	private void addRequestTypeGroup() {
		...
		requestTypeGroup
				.setTabList(new Control[] { this.customizingRequests, this.workbenchRequests, this.transportofCopies });
...
//		if (this.preferenceStoreModel.getFilterReleasedComboSelection() == 3
//				&& this.preferenceStoreModel.getToDateText() != null
//				&& !this.preferenceStoreModel.getToDateText().isEmpty()) {
//			int year = Integer.parseInt(PreferencesUtil.getYearFromDate(this.preferenceStoreModel.getToDateText()));
//			int month = Integer.parseInt(PreferencesUtil.getMonthFromDate(this.preferenceStoreModel.getToDateText()))
//					- 1;
...
//			this.setToReleasedDate(PreferencesUtil.calculateAndFormatDate(this.toDateTime.getYear(),
//					this.toDateTime.getMonth(), this.toDateTime.getDay()));
//		}

		if (this.preferenceStoreModel.getFilterReleasedComboSelection() == 0) {
			this.setToReleasedDate(PreferencesUtil.calculateAndFormatDate((Date) null, 0));
		} else if (1 == this.preferenceStoreModel.getFilterReleasedComboSelection()) {
			this.setToReleasedDate(PreferencesUtil.calculateAndFormatDate((Date) null, 0));
		} else if (2 == this.preferenceStoreModel.getFilterReleasedComboSelection()) {
			this.setToReleasedDate(PreferencesUtil.calculateAndFormatDate((Date) null, 0));
		} else if (3 == this.preferenceStoreModel.getFilterReleasedComboSelection()) {
			this.toDateTime.setEnabled(true);
			this.setToReleasedDate(this.preferenceStoreModel.getToDateText());
		}
		
		try {
			updateDateTimeControl(toDateTime, this.getToReleasedDate());
		} catch (Exception e) {
		}
		...
		this.toDateTime.addSelectionListener(new SelectionListener() {
			public void widgetSelected(SelectionEvent e) {
				SettingDialog.this.setToReleasedDate(
						PreferencesUtil.calculateAndFormatDate(SettingDialog.this.toDateTime.getYear(),
								SettingDialog.this.toDateTime.getMonth(), SettingDialog.this.toDateTime.getDay()));
...
//		if (this.preferenceStoreModel.getFilterReleasedComboSelection() == 3
//				&& this.preferenceStoreModel.getFromDateText() != null
//				&& !this.preferenceStoreModel.getFromDateText().isEmpty()) {
//			int year = Integer.parseInt(PreferencesUtil.getYearFromDate(this.preferenceStoreModel.getFromDateText()));
//			int month = Integer.parseInt(PreferencesUtil.getMonthFromDate(this.preferenceStoreModel.getFromDateText()))
//					- 1;
...
//			this.setFromReleasedDate(PreferencesUtil.calculateAndFormatDate(this.fromDateTime.getYear(),
//					this.fromDateTime.getMonth(), this.fromDateTime.getDay()));
//		}
		...
		if (this.preferenceStoreModel.getFilterReleasedComboSelection() == 0) {
			...
			this.fromDateTime.setEnabled(true);
			this.setFromReleasedDate(this.preferenceStoreModel.getFromDateText());
		}
		
		try {
			updateDateTimeControl(fromDateTime, this.getFromReleasedDate());
		} catch (Exception e) {
		}
		...
		this.fromDateTime.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				SettingDialog.this.setFromReleasedDate(
						PreferencesUtil.calculateAndFormatDate(SettingDialog.this.fromDateTime.getYear(),
								SettingDialog.this.fromDateTime.getMonth(), SettingDialog.this.fromDateTime.getDay()));
				SettingDialog.this.setErrorMessage((String) null);
				SettingDialog.this.enableApplyFilterButton(true);
			}
		});
	...
	private void addFilterReleasedCombo(Composite composite) {
		...
		this.filterReleasedDateCombo.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				// 更新日期控件
				try {
					updateDateTimeControl(SettingDialog.this.fromDateTime, SettingDialog.this.getFromReleasedDate());
					updateDateTimeControl(SettingDialog.this.toDateTime, SettingDialog.this.getToReleasedDate());
				} catch (Exception e) {
					e.printStackTrace();
				}
...
//	      }
		...
		this.userNameBrowseButton.addListener(13, new Listener() {
			public void handleEvent(Event event) {
				...
				if (SettingDialog.this.project != null) {
					String[] selectedUsers = adtUserServiceUI.openUserNameSelectionDialog(SettingDialog.this.getShell(),
							false, SettingDialog.this.project,
							SettingDialog.this.userNameTextField.getTextWidget().getText());
	...
	private void addUserNameTextField(Composite composite) {
		...
		FieldHistoryProposalProvider.create(this.userNameTextField.getTextWidget(),
				SettingDialog.class.getName() + ".user", true);
		...
		this.userNameStyledText.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				SettingDialog.this.setUserName(
						SettingDialog.this.userNameStyledText.getText().trim().toUpperCase(Locale.ENGLISH));
	...
	private void storeSelectedUIPreferences() {
		...
		this.preferenceStoreModel.setRequestObjectVisible(isRequestObjectVisible());
...
//	      }
	...
	public boolean isFromDateAfterToDate(Date fromDate, Date toDate) {
		if (fromDate.after(toDate)) {
			...
			if (fromDateCalendar.get(1) != toDateCalendar.get(1) || fromDateCalendar.get(2) != toDateCalendar.get(2)
					|| fromDateCalendar.get(6) != toDateCalendar.get(6)) {
				return true;
			}
		}
...
//	   }
	...
	private void updateDateTimeControl(DateTime dateTime, String date) throws Exception {
		int year = Integer.parseInt(PreferencesUtil.getYearFromDate(date));
		int month = Integer.parseInt(PreferencesUtil.getMonthFromDate(date)) - 1;
		int day = Integer.parseInt(PreferencesUtil.getDayFromDate(date));
